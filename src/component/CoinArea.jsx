  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900/95 to-gray-900/90 text-white px-4 sm:px-[5%] py-6 md:py-10 relative z-0">
      ...
      <div className="text-center mb-8 md:mb-12 space-y-4 group relative">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-yellow-500 to-orange-400 bg-clip-text text-transparent animate-gradient-x leading-tight">
          Crypto <br />
          <span className="text-xl sm:text-2xl md:text-3xl lg:text-4xl bg-gradient-to-r from-yellow-500 to-orange-500 bg-clip-text text-transparent">
            Market Intelligence
          </span>
        </h1>
        <p className="text-gray-300/80 max-w-xl mx-auto text-sm sm:text-base md:text-lg leading-relaxed mt-4">
          ...
          <br />
          <span className="bg-gradient-to-r from-yellow-200 to-yellow-600 bg-clip-text text-transparent mx-2">
      ...
      <div className="hidden md:grid grid-cols-5 gap-4 text-sm py-4 px-4 mb-2 bg-gray-800/40 backdrop-blur-lg rounded-xl border border-emerald-500/20">
        <p className="text-yellow-400/90">Rank</p>
        <p className="text-yellow-400/90">Coins</p>
        <div
          className="relative flex items-center gap-1 cursor-pointer group"
          onClick={() => setIsCurrencyDropdownOpen(!isCurrencyDropdownOpen)}
        >
          <span className="text-yellow-400/90">Price</span>
        ...
        <p className=" text-center text-yellow-400/90">24H Flux</p>
        <p className="text-right text-yellow-400/90">Market Cap</p>
      </div>
      {/* CURRENCY DROPDOWN */}
      {isCurrencyDropdownOpen && (
        <div className="relative bg-gray-800/95 backdrop-blur-xl rounded-lg border border-emerald-500/20 shadow-2xl z-50">
          {["usd", "eur", "inr"].map((code) => (
            <div
              key={code}
              className="px-4 py-3 hover:bg-yellow-500/30 transition-colors cursor-pointer flex items-center gap-2"
      ...
      <div className="space-y-3 relative z-10">
        {filteredCryptos.slice(0, 12).map((item) => (
          <Link
            to={`/crypto/${item.id}`}
            key={item.id}
            className="block p-4 bg-gray-800/30 backdrop-blur-md hover:bg-gray-700/40 rounded-xl border border-yellow-500/20 hover:border-orange-500/30 transition-all duration-300 group"
          >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <img
                    src={item.image}
                    alt={item.name}
                    className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-yellow-500/20 to-cyan-500/20 p-0.5"
                  />
                  <div>
                    <p className="font-medium text-gray-100 sm:text-base text-sm">
                      {item.name}
                    </p>
                    <p className="text-xs sm:text-sm text-yellow-400/80 mt-0.5">
            ...
            <div className="hidden md:grid grid-cols-5 gap-4 items-center">
              <span className="text-yellow-400/80 text-sm lg:text-base">
                #{item.market_cap_rank}
              </span>
              ...
              <div className="flex items-center gap-3">
                <img
                  src={item.image}
                  alt={item.name}
                  className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-gradient-to-br from-yellow-500/20 to-orange-500/20 p-0.5"
                />
                <div>
                  <p className="font-medium text-gray-100 text-base lg:text-lg">
                    {item.name}
                  </p>
                  <p className="text-sm lg:text-base text-yellow-400/80">
                    {item.symbol.toUpperCase()}
                  </p>
                </div>
              </div>
              ...
              <div className="text-right">
                  ...
                  <p className="text-xs lg:text-sm text-yellow-400/60  mt-0.5">
                    Vol : {currentCurrency.symbol}
                    {item.total_volume.toLocaleString()}
                  </p>
                </div>
